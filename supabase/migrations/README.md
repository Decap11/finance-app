...
Run **0001 through 0036 in numeric order, without stopping.**
...
Run **0015, then 0016, then 0017, then 0018, then 0019, then 0020, then 0021, then 0022, then 0023, then 0024, then 0025, then 0026, then 0027, then 0028, then 0029, then 0030, then 0031, then 0032, then 0033, then 0034, then 0035, then 0036**. 0016 depends on 0015's `saccos_update_admin_only`
...
| 0036 | `plan-catalogue-alignment` | 💳 **Makes the database name the plans the app actually sells.** `src/utils/subscriptionPlans.js` is the price list three separate surfaces read — the payments page, `/api/subscription-plans` and the checkout, which prices a request from the catalogue rather than from the request body — and it sells **basic** (free onboarding trial), **standard** (60,000/month, recommended) and **premium** (200,000 per 3 months). 0016 constrained `saccos.subscription_plan` to a different list entirely, `(basic, premium, enterprise)`, so the plan most SACCOs are meant to be on **could not be stored at all** while a tier nothing sells could; `subscriptionPlans.js` has carried a note about that since it was written. Migrates any `enterprise` row to `premium` — before the constraint moves, or `ADD CONSTRAINT` fails on it — and replaces the constraint with the catalogue. Also settles `subscription_amount`, which defaulted to **150,000, a price no plan has ever charged**: `register_new_sacco` does not set the column and nothing afterwards writes it (record-payment touches only the status and expiry; update-subscription writes an amount only when a caller supplies one, which the portal never did), so the figure the developer portal billed every tenant against was the column default and nothing else. The default becomes 0 and rows sitting on 150,000 or NULL are backfilled to their plan's real price; **a row holding any other figure was set by hand and is left alone.** The column is per *billing term*, not per month — premium is one payment covering three, and a stored monthly figure would round and stop reconciling against what was collected — so anything wanting a monthly rate divides by `durationMonths` at the point of display (`planMonthlyPrice()`), which is what stops the portal's platform-income metric counting a quarterly tenant at three times what it earns. The application half is the developer portal, which no longer carries its own price table, and `/api/platform`, which validates against `PLAN_IDS`. `verify-schema.sql` check 17 fails on the un-widened constraint or a surviving enterprise row, and warns while the 150,000 default is still in place. Depends on 0016 and 0032. |
